...
jbergstroem/mariadb-alpine の派生みたいなもの
...
起動方法 データ永続化時（カレントディレクトリ my_data 配下に保存）

```console
$ docker run --name db -it --rm --init -v $(pwd)/my_data:/var/lib/mysql -p 3306:3306 tiny_mysql
```

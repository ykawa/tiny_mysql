...
# 不要なファイルを削除する
apk info -q -L mariadb mariadb-common mariadb-client \
  | awk 'BEGIN{while((getline v<"/tmp/keep_file_list")>0){a[v]=1}}{if(!a[$0]){print $0}}' \
  | xargs rm -f

# linuxのdocker環境を考慮してmysqlユーザーのidを変更する
sed -ie 's/mysql:x:[0-9]*:mysql/mysql:x:'${MY_GROUP_ID:-101}':mysql/' /etc/group
sed -ie 's/mysql:x:[0-9]*:[0-9]*:/mysql:x:'${MY_USER_ID:-100}':'${MY_GROUP_ID:-101}':/' /etc/passwd
